"""Тесты выбора каталога и предзаполнения папки по умолчанию (checker.picker)."""
import os
import subprocess

import pytest

from checker import picker
...
# --------------------------------------------------------------------------- #
# _win_folder_dialog (не-GUI части: передача FSCHK_INITIAL через env, вывод)
# --------------------------------------------------------------------------- #
def test_win_folder_dialog_passes_initial_and_title_via_env(monkeypatch):
    ...
    assert env["FSCHK_INITIAL"] == r"C:\Users\me\proj"
    assert env["FSCHK_TITLE"]  # заголовок передаётся в скрипт
    # WSLENV нужен, чтобы переменные дошли до powershell.exe из-под WSL.
    wslenv = env["WSLENV"].split(":")
    assert "FSCHK_INITIAL" in wslenv
    assert "FSCHK_TITLE" in wslenv
...
def test_win_folder_dialog_without_initial_clears_env(monkeypatch):
    monkeypatch.setattr(picker.shutil, "which", lambda name: "/fake/powershell.exe")
    monkeypatch.setenv("FSCHK_INITIAL", "stale")
...
def test_win_folder_dialog_preserves_existing_wslenv(monkeypatch):
    # Уже объявленные в WSLENV переменные не должны теряться при добавлении FSCHK_*.
    monkeypatch.setattr(picker.shutil, "which", lambda name: "/fake/powershell.exe")
...
def test_mac_folder_dialog_returns_path_and_passes_initial(monkeypatch):
    ...
    env = captured["env"]
    assert isinstance(env, dict)
    assert env["FSCHK_INITIAL"] == "/Users/me/proj"
    cmd = captured["cmd"]
    assert isinstance(cmd, list)
