"""Выбор каталога для проверки (диалоги Windows/WSL/macOS, ввод в терминале)."""
...
def _set_initial(env: dict[str, str], initial: str | None) -> None:
    """Кладёт стартовую папку в FSCHK_INITIAL или убирает устаревшее значение."""
    if initial:
        env["FSCHK_INITIAL"] = initial
    else:
        env.pop("FSCHK_INITIAL", None)  # не подхватывать устаревшее значение
...
def _win_folder_dialog(initial: str | None = None) -> str | None:
    ...
    Стартовая папка и заголовок передаются через env (FSCHK_INITIAL, FSCHK_TITLE),
    ...
    try:
        ...
        return None
    env = os.environ.copy()
    env["FSCHK_TITLE"] = "Выберите каталог для проверки"
    ...
    adds = ["FSCHK_TITLE"] + (["FSCHK_INITIAL"] if initial else [])
    ours = {"FSCHK_TITLE", "FSCHK_INITIAL"}
...
def _mac_folder_dialog(initial: str | None = None) -> str | None:
    ...
    if not osascript:
        return None
    prompt = "Выберите каталог для проверки"
    script = (
        'set p to (system attribute "FSCHK_INITIAL")\n'
        'if p is not "" then\n'
...
def pick_directory() -> str:
    ...
    return _prompt_directory("Укажите каталог для проверки.", default=cwd)
