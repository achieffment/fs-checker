...
> Утилита доступна в составе пакета: **[fs-tools](https://github.com/achieffment/fs-tools)**.
...
- Чтение правил из `.fs-check` в выбранном каталоге (кодировка `utf-8-sig`). Нет
...
В [`examples/`](examples/) — запускаемое дерево-фикстура с `.fs-check` и намеренно
